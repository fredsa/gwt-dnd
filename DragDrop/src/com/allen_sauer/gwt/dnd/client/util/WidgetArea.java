...
public class WidgetArea extends AbstractArea {
  ...
  /**
   * Determine the area of a widget relative to a panel. The area returned is such that the
   * following are true:
   * <ul>
   * <li><code>parent.add(widget, area.getLeft(), area.getTop())</code> leaves the object in the
   * exact same location on the screen</li>
   * <li>
   * <code>area.getRight() = area.getLeft() + widget.getOffsetWidget()</code></li>
   * <li>
   * <code>area.getBottom() = area.getTop() + widget.getOffsetHeight()</code></li>
   * </ul>
   * 
   * Note that boundaryPanel need not be the parent node, or even an ancestor of widget. Therefore
   * coordinates returned may be negative or may exceed the dimensions of boundaryPanel.
   * 
   * @param widget the widget whose area we seek
   * @param reference the widget relative to which we seek our area. If <code>null</code>, then
   *          <code>RootPanel().get()</code> is assumed
   */
  // public WidgetArea(Widget widget, Widget reference) {
  // setLeft(widget.getAbsoluteLeft());
  // setTop(widget.getAbsoluteTop());
